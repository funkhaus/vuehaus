<?php
/*
 * This file handles the required plugins of the vuehaus theme
 */
    ...
    function vuehaus_register_required_plugins() {
        ...
        $latest_rest_easy = '1.48';
        $latest_focushaus = '1.0.5';

        $config = array(
            'id'           => 'vuehaus',              // Unique ID for hashing notices for multiple instances of TGMPA.
    ...
    function get_custom_template_routes(){
		...
		// Find all pages with custom_vuehaus_template set
		$args = array(
			'posts_per_page'   => -1,
			'orderby'          => 'menu_order',
			'order'            => 'ASC',
			'meta_key'         => 'custom_vuehaus_template',
			'meta_value'       => '',
			'post_type'        => array('post', 'page')
		);
		$all_pages_with_custom_templates = get_posts($args);

		// Filter out pages with vuehaus custom template set to Default
		$filtered_pages_with_custom_templates = array_filter(
			$all_pages_with_custom_templates,
			function( $page ){
				return $page->custom_vuehaus_template !== 'Default';
			}
		);
		...
		foreach( $filtered_pages_with_custom_templates as $page ){
			...
			$custom_template_routes[wp_make_link_relative($page_url)] = $page->custom_vuehaus_template;
		}
